...
@skill(
    name="timer_set",
    risk="low",
    help_text="Lance un minuteur ou un rappel depuis une durée en secondes",
    describe=lambda d: f"Lancer un {d.get('kind', 'timer')} de {d.get('duration_s', 0)} secondes",
)
async def skill_timer_set(data: dict) -> str:
    duration_s = int(data.get("duration_s", 0) or 0)
    return await skill_timer(
        {
            "minutes": duration_s // 60,
            "secondes": duration_s % 60,
