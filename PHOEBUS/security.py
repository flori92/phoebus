# PHOEBUS/security.py
"""Sécurité PHOEBUS — audit, authentification WS, confirmations vocales."""
import json
from datetime import datetime

from PHOEBUS.config import AUDIT_FILE, DEVICE_CONFIG_FILE
from PHOEBUS.utils import normalize_text

# ── Actions sensibles ──────────────────────────────────────────────────────
# Trois niveaux de risque :
#  - "low"    : on exécute silencieusement.
#  - "medium" : on exécute, mais PHOEBUS annonce "J'applique X" avant de le faire
...
def describe_action(data):
    ...
    descriptions = {
        ...
        "create_doc":      lambda d: f"creer le document {d.get('title', 'Document PHOEBUS')}",
        "write_doc":       lambda _: "modifier le document Google en cours",
        "create_sheet":    lambda d: f"creer la feuille {d.get('title', 'Feuille PHOEBUS')}",
    }
...
def risk_level_for(action):
    ...
    try:
        from PHOEBUS.skills import risk_of
