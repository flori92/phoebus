"""Back-ends de synthèse vocale (TTS) de PHOEBUS.
...
Quatre back-ends supportés, sélectionnables via `PHOEBUS_TTS_BACKEND` :
...
from PHOEBUS.config import edge_tts
from PHOEBUS.lipsync import build_lipsync_frames_from_word_boundaries
...
PIPER_MODEL = os.getenv("PHOEBUS_PIPER_MODEL", "").strip()
PIPER_CONFIG = os.getenv("PHOEBUS_PIPER_CONFIG", "").strip()
# Débit Piper : 1.0 = normal ; >1.0 = plus lent (inverse de edge-tts).
PIPER_LENGTH_SCALE = float(os.getenv("PHOEBUS_PIPER_LENGTH", "0.95"))
PIPER_NOISE_SCALE = float(os.getenv("PHOEBUS_PIPER_NOISE", "0.667"))
PIPER_NOISE_W = float(os.getenv("PHOEBUS_PIPER_NOISE_W", "0.8"))

PHOEBUS_TTS_BACKEND = os.getenv("PHOEBUS_TTS_BACKEND", "auto").strip().lower()
...
def _backend_actif() -> str:
    if PHOEBUS_TTS_BACKEND == "piper":
        return "piper"
    if PHOEBUS_TTS_BACKEND == "eleven":
        return "eleven"
    if PHOEBUS_TTS_BACKEND == "edge":
        return "edge"
