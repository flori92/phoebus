# PHOEBUS/automation.py
"""Moteur d'automatisation proactif et routines de fond de PHOEBUS."""
import time
import asyncio
import threading
...
from PHOEBUS.home import ha_get_etat, ha_lumiere, PIECES_LUMIERES
from PHOEBUS.voice import parler
from PHOEBUS.desktop import action_whatsapp_appel
from PHOEBUS.config import pyautogui, IS_WINDOWS, client, Image, CHOSEN_MODEL, types
...
async def verifier_vision_passive():
    ...
    if now - DERNIER_SCAN_VISION > 300:
        ...
        try:
            import base64
            path_ss = "PHOEBUS_passive_vision.png"
...
async def boucle_automatisation():
    ...
    while True:
        try:
            # Ne pas interrompre si PHOEBUS parle ou écoute déjà
...
def demarrer_moteur_automatisation():
    ...
    t = threading.Thread(target=run_loop, daemon=True, name="PHOEBUSAutomation")
    t.start()
