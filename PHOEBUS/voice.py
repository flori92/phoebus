...
from PHOEBUS.config import edge_tts, pygame, pyaudio, sr, CLAP_THRESHOLD, pyautogui
import PHOEBUS.state as state
from PHOEBUS.home import resolve_ha_entity, PIECES_LUMIERES, ha_get_etat, ha_lumiere
from PHOEBUS.tts_backends import synthesize_to_file, TtsUnavailable, EDGE_VOICE
...
def resoudre_francais_localement(texte):
    ...
    dic = {
        ...
        "PHOEBUS": "Just A Rather Very Intelligent System.",
    }
...
async def _send_text_fallback(text):
    ...
    if recipients and text:
        msg = json.dumps({"action": "PHOEBUS_response", "text": text}, ensure_ascii=False)
...
async def _prepare_tts(sentence):
    ...
    tmp = f"PHOEBUS_tts_{int(time.time()*1_000_000)}.mp3"
    try:
        ...
        if state._skip_pc_audio:
            ...
            if recipients:
                try:
                    ...
                    msg = json.dumps({"action": "PHOEBUS_audio", "text": texte_tts, "audio_b64": audio_b64})
                    await asyncio.gather(*[ws.send(msg) for ws in recipients], return_exceptions=True)
                except Exception as e:
            ...
            if not init_mixer():
                recipients = state.get_authenticated_clients()
                if recipients:
                    msg = json.dumps({"action": "PHOEBUS_response", "text": texte_tts})
                    await asyncio.gather(*[ws.send(msg) for ws in recipients], return_exceptions=True)
                return
...
async def parler(texte, keep_conversation=True):
    ...
    state.current_PHOEBUS_speech = texte_tts
    ...
    try:
        ...
        while True:
            try:
                ...
                # ── Barge-in : si PHOEBUS parle et on détecte de la voix forte,
                ...
                if state.is_speaking:
                    if rms > state.BARGE_IN_THRESHOLD:
                        ...
                        barge_in_streak = 0
                    # On ne fait pas de clap-detection pendant que PHOEBUS parle.
                    last_clap_time = 0
                    continue
                else:
                    barge_in_streak = 0
