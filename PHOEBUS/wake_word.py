...
Modèle Vosk FR (recommandé pour option 2) :
  ...
  # Puis dans .env :
  PHOEBUS_VOSK_MODEL_PATH=/chemin/vers/vosk-model-small-fr-0.22
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
import os
import time
...
logger = logging.getLogger("PHOEBUS.wake_word")
...
OWW_THRESHOLD     = float(os.getenv("PHOEBUS_OWW_THRESHOLD", "0.5"))
# Cooldown minimum entre deux déclenchements (secondes)
WAKE_COOLDOWN_S   = float(os.getenv("PHOEBUS_WAKE_COOLDOWN", "2.0"))
...
def _resolve_vosk_path() -> str:
    ...
    """
    raw = os.getenv("PHOEBUS_VOSK_MODEL_PATH", "").strip()
    if not raw:
...
VOSK_KEYWORDS     = [
    "phoebus", "phébus", "fébus", "febus", "feubus", "rebus",
    "hey phoebus", "ok phoebus", "allo phoebus",
]
...
WAKE_WORDS_PHONETIC = [
    "phoebus", "phébus", "fébus", "febus", "feubus", "rebus",
    "hey phoebus", "ok phoebus", "yo phoebus", "allo phoebus",
]
...
def start(callback):
    ...
    if _thread and _thread.is_alive():
        return
    _thread = threading.Thread(target=_run, daemon=True, name="PHOEBUSWakeWord")
    _thread.start()
