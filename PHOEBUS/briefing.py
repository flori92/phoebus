# PHOEBUS/briefing.py
"""
Briefing matinal automatique de PHOEBUS.

Chaque matin (heure configurable), PHOEBUS prend la parole de lui-même et
...
Variables d'environnement :
  PHOEBUS_BRIEFING_HOUR=8         → heure du briefing (défaut : 8h)
  PHOEBUS_BRIEFING_MUSIC=1        → lance une playlist de réveil (défaut : 0)
  PHOEBUS_BRIEFING_NEWS=1         → inclut les actualités (défaut : 1)
  PHOEBUS_BRIEFING_PLAYLIST=      → nom de la playlist Spotify de réveil
"""
import os
import asyncio
...
BRIEFING_HOUR      = int(os.getenv("PHOEBUS_BRIEFING_HOUR", "8"))
BRIEFING_MUSIC     = os.getenv("PHOEBUS_BRIEFING_MUSIC", "0").strip() == "1"
BRIEFING_NEWS      = os.getenv("PHOEBUS_BRIEFING_NEWS", "1").strip() == "1"
BRIEFING_PLAYLIST  = os.getenv("PHOEBUS_BRIEFING_PLAYLIST", "").strip()
...
async def _get_meteo_snippet() -> str:
    try:
        from PHOEBUS.home import get_meteo_actuelle, get_alertes_meteo
...
async def _get_rag_snippet() -> str:
    ...
    try:
        from PHOEBUS.rag_memory import rechercher_souvenirs
...
async def verifier_et_lancer_briefing(parler_fn) -> bool:
    ...
    if BRIEFING_MUSIC:
        try:
            from PHOEBUS.spotify import jouer as spotify_jouer
