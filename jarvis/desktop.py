# jarvis/desktop.py
"""Agent desktop JARVIS — gestion fichiers, apps, YouTube, WhatsApp, volume."""
import os
import re
import time
import math
...
from jarvis.config import EXTENSIONS, pyautogui, IS_WINDOWS
from jarvis.utils import special_folder, open_path, launch_app, desktop_file, open_uri
...
def executer_action_pc(commande):
    ...
    if "eteins" in cmd or "shutdown" in cmd:
        if "confirme" not in cmd and "confirm" not in cmd:
            return "Commande sensible. Dites 'Jarvis confirme extinction' pour eteindre cette machine."
        from jarvis.utils import shutdown_system
        shutdown_system(5)
...
async def action_whatsapp_appel(contact, parler_fn):
    ...
    try:
        ...
        from jarvis.vision import jarvis_vision_cliquer
