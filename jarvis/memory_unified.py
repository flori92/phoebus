"""Façade unifiée sur toutes les couches de mémoire de Jarvis.
...
from jarvis.rag_memory import stocker_souvenir, rechercher_souvenirs
...
def note_correction(original: str, corrected: str) -> None:
    ...
    Exemple : Jarvis a dit "il fait 25°C à Paris" → utilisateur répond
    ...
    payload = (
        ...
        f"Réponse initiale de Jarvis : « {original.strip()[:250]} ». "
        f"Correction de Floriace : « {corrected.strip()[:250]} ». "
        f"Jarvis doit retenir la correction et s'y conformer."
    )
