"""Routeur de cerveau IA pour JARVIS.
...
PROVIDERS = ("gemini", "groq", "openai", "mistral", "kimi", "grok", "ollama")
...
def configured_order() -> list[str]:
    return _csv(os.getenv("JARVIS_BRAIN_ORDER", ""), DEFAULT_ORDER)


def configured_mode() -> str:
    mode = os.getenv("JARVIS_BRAIN_MODE", "balanced").strip().lower()
...
def build_profile(texte: str, streaming: bool = False, in_conversation: bool = False) -> BrainProfile:
    ...
    if streaming or in_conversation or kind == "command":
        priority = "fast"
        timeout = float(os.getenv("JARVIS_BRAIN_FAST_TIMEOUT", "7"))
    elif kind == "deep":
        priority = "smart"
        timeout = float(os.getenv("JARVIS_BRAIN_SMART_TIMEOUT", "14"))
    else:
        priority = "balanced"
        timeout = float(os.getenv("JARVIS_BRAIN_TIMEOUT", "10"))
